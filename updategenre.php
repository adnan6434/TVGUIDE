...
{   $gid=$_POST['id'];
	$genname=mysqli_real_escape_string($conn,$_POST['catname']);

	$sql = "UPDATE `genre` SET `GenreName` = '$genname' WHERE `genre`.`GenreID` = $gid";
...
?>
<form id="add_form" method="POST" action="updategenre.php">
	<fieldset>
	...
	<?php if(isset($_GET['error'])): ?>
		...
		<p>
			...
			<input id="catname"  name="catname" type="text" required="true" val value="<?php echo $name;?>" />
		</p>

<p>
	<input type="submit" value="update" name="updategenre">
</p>
</fieldset>
	</form>
