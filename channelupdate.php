...
$f=mysqli_fetch_assoc($r);
    $cid=$f['ChannelID'];
    $name=$f['ChannelName'];
	$des=$f['ChannelDescription'];
	$logo=$f['ChannelLOGOUrl'];
	$cat=$f['ChannelCategoryID'];
}
if(isset($_POST['updatechannel']))
{    $Chid=$_POST['cid'];
	$Ch=mysqli_real_escape_string($conn,$_POST['channelname']);
	$des=mysqli_real_escape_string($conn,$_POST['channeldescription']);
	$clogo=mysqli_real_escape_string($conn,$_POST['channellogourl']);
	$cate=$_POST['channelcategoryid'];
	$sql = "UPDATE `channel` SET `ChannelName` = '$Ch', `ChannelDescription` = '$des', `ChannelLOGOUrl` = '$clogo', `ChannelCategoryID` = $cate WHERE `channel`.`ChannelID` = $Chid  ";
...
?>
<form id="update_channel" method="POST" action="channelupdate.php">
	<fieldset>
	...
	<?php if(isset($_GET['error'])): ?>
			...
			<p>
			<input id="id"  name="cid" type="hidden" required="true" 
			 value="<?php echo $cid;?>" />
		</p>
		<p>
			<label for="channelname">ChannelName</label>
			<input id="channelname"  name="channelname" type="text" required="true" value="<?php echo $name;?>"/>
		</p>
		<p>
			<label for="channeldescription">Channel Description</label>
			<input id="channeldescription"  name="channeldescription" type="text" required="true" value="<?php echo $des;?>"/>
		</p>
		<p>
			<label for="channellogourl">LOGOUrl</label>
			<input id="channellogourl"  name="channellogourl" type="text" required="true" value="<?php echo $logo;?>" />
		...
		<p>
			...
			$r=mysqli_query($conn,$sql) ;
			$f=mysqli_fetch_assoc($r);
			echo "<input type='number' name='channelcategoryid' required='true' min='1' max='".implode("",$f)."'step='1'value='".$cat."'></p>";
			?>  
			
		</p>

<p>
	<input type="submit" value="Update channel" name="updatechannel" style="background:yellow;">
</p>
</fieldset>
	</form>
