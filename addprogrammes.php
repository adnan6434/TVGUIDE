...
if(isset($_POST['addprogramme']))
{
	...
	$ts=$_POST['timeslotid'];
	$sql="INSERT INTO `programme` (`ChannelID`, `ProgrameName`, `GenreID`, `ProgrammeDescription`, `ProgrammeTime`, `TimeSlotID`) VALUES ('$ch','$pname','$gid','$des','$ptime','$ts')";
...
?>
<form id="add_programme" method="POST" action="addprogrammes.php">
	<fieldset>
	<legend>ADD PROGRAMMES</legend>
	...
	<?php if(isset($_GET['error'])): ?>
		  ...
		  <p>
			...
			echo "<input type='number' name='channelid' required='true' min='1' max='".implode("",$f)."'step='1'></p>";
			?>          <p>
			...
			$r=mysqli_query($conn,$sql) ;
			$f=mysqli_fetch_assoc($r);
			echo "<input type='number' name='genreid' required='true' min='1' max='".implode("",$f)."'step='1'></p>";
			?>
		<p>
			<label for="programmename">Programme Name</label>
			<input id="programmename"  name="programmename" type="text" required="true"/>
		
		<p>
			<label for="programmedescription">Programme Description</label>
			<input id="programmedescription"  name="programmedescription" type="text" required="true"/>
		</p>
		<p>
			<label for="programmetime">programmeTime</label>
			<input id="programmetime"  name="programmetime" type="datetime"  required="true" />
	  ...
	  <p>
			...
			$r=mysqli_query($conn,$sql) ;
			$f=mysqli_fetch_assoc($r);
			echo "<input type='number' name='timeslotid' required='true' min='1' max='".implode("",$f)."'step='1'></p>";
			?>

<p>
	<input type="submit" value="addprogramme" name="addprogramme">
</p>
</fieldset>
	</form>
